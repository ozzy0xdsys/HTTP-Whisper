    ...
    runtime.block_on(async move {
        ...
        if current.enable_https_interception && current.auto_install_ca {
            let _ = events.send(CaptureEvent::Log(
                "Installing HTTP Whisper CA in the current-user trust store".into(),
            ));
            install_current_user_ca(&certificates.certificate_der)?;
            let _ = events.send(CaptureEvent::Log("HTTP Whisper CA trusted".into()));
        }

        let mut system_proxy = WindowsProxyManager::new(paths.data_dir.join("proxy-restore.json"));
        if system_proxy.recover_if_needed()? {
            let _ = events.send(CaptureEvent::Log(
                "Recovered system proxy settings from an interrupted capture".into(),
            ));
        }
        if current.auto_configure_system_proxy {
            system_proxy.enable(&current.capture_host, current.capture_port)?;
            let _ = events.send(CaptureEvent::Log(format!(
                "System and Firefox proxy configured: {}",
                system_proxy.summary()?
            )));
        }
